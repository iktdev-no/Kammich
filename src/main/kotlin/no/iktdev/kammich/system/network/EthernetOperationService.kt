...
import no.iktdev.kammich.models.shared.network.EthernetInterfaceStateType
...
@Service
class EthernetOperationService(
    ...
    fun getCurrentState(interfaceName: String): EthernetInterfaceStateType {
        val iface = interfaceRegistry.findInterface(name = interfaceName)
            ?: return EthernetInterfaceStateType.Disconnected

        val strategy = getActiveStrategy() ?: return EthernetInterfaceStateType.Disconnected
        val carrier = strategy.hasCarrier(interfaceName)

        if (carrier == false) return EthernetInterfaceStateType.Disconnected

        return when (iface.mode) {
            NetworkInterfaceMode.Client ->
                if (!strategy.getDeviceIpv4Addresses(interfaceName).isNullOrBlank())
                    EthernetInterfaceStateType.Connected
                else
                    EthernetInterfaceStateType.Connecting

            NetworkInterfaceMode.Tether ->
                EthernetInterfaceStateType.Emergency

            else ->
                EthernetInterfaceStateType.Disconnected
        }
    }

    fun getConnection(interfaceName: String): EthernetInterfaceState? {
        val iface = interfaceRegistry.findInterface(name = interfaceName) ?: return null
        val strategy = getActiveStrategy() ?: return null

        return EthernetInterfaceState(
            ...
            operatingMode = strategy.getState(interfaceName),
            error = null
        )
    }

    fun getConnections(): List<EthernetInterfaceState> =
    ...
    fun setEmergencyAllowed(interfaceName: String, allowed: Boolean): Boolean {
        ...
        getConnection(interfaceName)?.let {
            sseManager.send(SSEEthernetState(interfaceName, it))
        }
        return true
    }
            ...
            sseManager.send(
                SSEEthernetState(
                    ...
                    EthernetInterfaceState(
                        ...
                        state = EthernetInterfaceStateType.Disconnected,
                        carrier = carrier,
                        ipv4 = ipv4,
                        emergencyAllowed = isEmergencyAllowed(it.interfaceName),
                        emergencyAt = null,
                        operatingMode = strategy.getState(it.interfaceName)
                    )
                )
            )
        }
